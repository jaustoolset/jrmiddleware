										  ...
										  MsgVersion version)
{
    ...
    if (getAddrFromId(id, prevAddr))
	{
		...
		// and return success.  However, firing a warning
        // if we're changing address
        if (prevAddr != addr)
            JrWarn << "Detected address change for JAUS ID " 
                   << id.val 
                   << ".  This could be normal operation, but may also indicate multiple "
                   << "devices with the same JAUS ID exist on the network" << std::endl;
